...
const MasterTahun = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await api.tahun.getAll();
  ...
  const handleAdd = async () => {
    ...
    try {
      const res = await api.tahun.create(newNama);
  ...
  const handleUpdate = async (id: number) => {
    ...
    try {
      const res = await api.tahun.update(id, editNama);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await api.tahun.delete(id);
  ...
  return (
    <div className="bg-white p-6 shadow-sm border border-slate-200">
      <div className="flex justify-between items-center mb-2">
        <h2 className="text-xl font-black text-ppm-green uppercase tracking-tight">Master Data Tahun</h2>
      ...
      <div className="mb-4 flex justify-end">
        <div className="relative w-full sm:w-72">
          ...
          <input type="text" placeholder="Cari tahun..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-9 pr-3 py-2 border border-slate-300 text-sm outline-none focus:border-ppm-green transition-colors" />
        </div>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr>
                  ...
                  <th className="table-header">{getLabel('master_tahun', 'nama', 'Nama Tahun')}</th>
              ...
              <tbody>
                {isAdding && (
                  <tr className="bg-blue-50">
                    ...
                    <td className="p-2 border-b border-slate-100">
                      <input autoFocus type="text" className="w-full border-2 border-ppm-green p-1.5 text-sm outline-none" placeholder="Masukkan tahun..." value={newNama} onChange={e => setNewNama(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleAdd()} />
                    </td>
                    <td className="p-2 border-b border-slate-100">
                ...
                {displayed.length === 0 && !isAdding && (
                  <tr><td colSpan={3} className="p-12 text-center text-slate-400 font-bold italic">{search ? 'Tidak ada hasil pencarian' : 'Belum ada data tahun'}</td></tr>
                )}
              </tbody>
            </table>
          </div>
          {/* Bottom: Tampilkan + info | Pagination */}
