...
const MasterJenisKegiatan = () => {
    const { getLabel } = useLabels();
    const [data, setData] = useState<{ id: number; nama: string }[]>([]);
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const res = await api.jenisKegiatan.getAll();
    ...
    const handleAdd = async () => {
        ...
        try {
            const res = await api.jenisKegiatan.create(newNama);
    ...
    const handleUpdate = async (id: number) => {
        ...
        try {
            const res = await api.jenisKegiatan.update(id, editNama);
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const res = await api.jenisKegiatan.delete(id);
    ...
    return (
        <div className="bg-white p-6 shadow-sm border border-slate-200">
            <div className="flex justify-between items-center mb-2">
                <h2 className="text-xl font-black text-ppm-green uppercase tracking-tight">Master Data Jenis Kegiatan</h2>
                <button onClick={() => setIsAdding(true)} className="bg-ppm-green text-white px-4 py-2 text-xs font-bold uppercase tracking-wider flex items-center gap-2 hover:bg-ppm-light-green transition-colors">
                    <Plus size={14} /> Tambah Jenis Kegiatan
                </button>
            </div>
            ...
            <div className="mb-4 flex justify-end">
                <div className="relative w-full sm:w-72">
                    ...
                    <input type="text" placeholder="Cari jenis kegiatan..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-9 pr-3 py-2 border border-slate-300 text-sm outline-none focus:border-ppm-green transition-colors" />
                </div>
            </div>
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr>
                                    ...
                                    <th className="table-header">{getLabel('master_jenis_kegiatan', 'nama', 'Nama Jenis Kegiatan')}</th>
                            ...
                            <tbody>
                                {isAdding && (
                                    <tr className="bg-blue-50">
                                        ...
                                        <td className="p-2 border-b border-slate-100">
                                            <input autoFocus type="text" className="w-full border-2 border-ppm-green p-1.5 text-sm outline-none" placeholder="Masukkan jenis kegiatan..." value={newNama} onChange={e => setNewNama(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleAdd()} />
                                        </td>
                                        <td className="p-2 border-b border-slate-100">
                                ...
                                {displayed.map((item, index) => (
                                    <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className="p-3 border-b border-slate-100 font-bold text-ppm-green">
                                            {editingId === item.id ? (
                                                <input autoFocus type="text" className="w-full border-2 border-ppm-green p-1.5 text-sm outline-none" value={editNama} onChange={e => setEditNama(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleUpdate(item.id)} />
                                            ) : item.nama}
                                        </td>
                                        <td className="p-3 border-b border-slate-100">
                                            <div className="flex justify-center gap-4">
                                                {editingId === item.id ? (
                                                    ...
                                                    <>
                                                        <button onClick={() => { setEditingId(item.id); setEditNama(item.nama); }} className="text-blue-600 hover:text-blue-800 p-1 transition-colors"><Edit2 size={16} /></button>
                                ...
                                {displayed.length === 0 && !isAdding && (
                                    <tr><td colSpan={3} className="p-12 text-center text-slate-400 font-bold italic">{search ? 'Tidak ada hasil pencarian' : 'Belum ada data jenis kegiatan'}</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
...
export default MasterJenisKegiatan;
