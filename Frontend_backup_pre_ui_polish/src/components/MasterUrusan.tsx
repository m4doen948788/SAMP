...
const MasterUrusan = () => {
    ...
    const [data, setData] = useState<{ id: number; urusan: string }[]>([]);
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const res = await api.urusan.getAll();
    ...
    const filtered = useMemo(() => {
        ...
        return data.filter(d => d.urusan.toLowerCase().includes(q));
    }, [data, search]);
    ...
    const handleAdd = async () => {
        ...
        try {
            const res = await api.urusan.create(newUrusan);
            if (res.success) { setNewUrusan(''); setIsAdding(false); fetchData(); }
    ...
    const handleUpdate = async (id: number) => {
        ...
        try {
            const res = await api.urusan.update(id, editUrusan);
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const res = await api.urusan.delete(id);
    ...
    return (
        <div className="bg-white p-6 shadow-sm border border-slate-200">
            <div className="flex justify-between items-center mb-2">
                <h2 className="text-xl font-black text-ppm-green uppercase tracking-tight">Master Data Urusan</h2>
            ...
            <div className="mb-4 flex justify-end">
                <div className="relative w-full sm:w-72">
                    ...
                    <input type="text" placeholder="Cari urusan..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-9 pr-3 py-2 border border-slate-300 text-sm outline-none focus:border-ppm-green transition-colors" />
                </div>
            </div>
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr>
                                    ...
                                    <th className="table-header">{getLabel('master_urusan', 'urusan', 'Urusan')}</th>
                            ...
                            <tbody>
                                {isAdding && (
                                    <tr className="bg-blue-50">
                                        <td className="p-3 border-b border-slate-100 text-slate-400">NEW</td>
                                        <td className="p-2 border-b border-slate-100">
                                            <input autoFocus type="text" className="w-full border-2 border-ppm-green p-1.5 text-sm outline-none" placeholder="Masukkan urusan..." value={newUrusan} onChange={e => setNewUrusan(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleAdd()} />
                                        </td>
                                        <td className="p-2 border-b border-slate-100">
                                ...
                                {displayed.map((item, index) => (
                                    <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className="p-3 border-b border-slate-100 font-bold text-ppm-green">
                                            {editingId === item.id ? (
                                                <input autoFocus type="text" className="w-full border-2 border-ppm-green p-1.5 text-sm outline-none" value={editUrusan} onChange={e => setEditUrusan(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleUpdate(item.id)} />
                                            ) : item.urusan}
                                        </td>
                                        <td className="p-3 border-b border-slate-100">
                                            <div className="flex justify-center gap-4">
                                                {editingId === item.id ? (
                                                    ...
                                                    <>
                                                        <button onClick={() => { setEditingId(item.id); setEditUrusan(item.urusan); }} className="text-blue-600 hover:text-blue-800 p-1 transition-colors"><Edit2 size={16} /></button>
                                ...
                                {displayed.length === 0 && !isAdding && (
                                    <tr><td colSpan={3} className="p-12 text-center text-slate-400 font-bold italic">{search ? 'Tidak ada hasil pencarian' : 'Belum ada data urusan'}</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
