...
const upload = multer({
    ...
    { name: 'surat_perintah', maxCount: 10 },
...
const syncDocumentType = async (connection, docId, field) => {
    try {
        ...
        // 1. If it's a letter (surat_undangan_masuk, surat_undangan_keluar, or surat_perintah)
        if (field === 'surat_undangan_masuk' || field === 'surat_undangan_keluar' || field === 'surat_perintah') {
            ...
            // Update the jenis_dokumen_id in dokumen_upload to match "Surat Undangan Masuk", "Surat Undangan Keluar", or "Surat Perintah"
            let keyword = '%undangan keluar%';
            if (field === 'surat_undangan_masuk') keyword = '%undangan masuk%';
            else if (field === 'surat_perintah') keyword = '%perintah%';
...
const create = async (req, res) => {
    ...
    try {
        ...
        const allFields = ['surat_undangan_masuk', 'surat_undangan_keluar', 'surat_perintah', 'bahan_desk', 'paparan', 'notulensi', 'foto', 'laporan'];
...
const resolveFilePath = (docPath) => {
    if (!docPath) return null;
    let clean = docPath.replace(/\\/g, '/');
    if (clean.startsWith('/')) clean = clean.slice(1);

    // candidate 1: Backend/uploads/...
    const candidate1 = clean.startsWith('uploads/')
        ? path.join(__dirname, '../../../../', clean)
        : path.join(__dirname, '../../../../uploads', clean);
    if (fs.existsSync(candidate1)) return candidate1;

    // candidate 2: root/uploads/...
    const candidate2 = clean.startsWith('uploads/')
        ? path.join(__dirname, '../../../../../', clean)
        : path.join(__dirname, '../../../../../uploads', clean);
    if (fs.existsSync(candidate2)) return candidate2;

    // candidate 3: VPS persistent storage path
    const candidate3 = path.join('/home/ppm/uploads_ppm_storage', clean.replace(/^uploads\//, ''));
    if (fs.existsSync(candidate3)) return candidate3;

    return candidate1;
};

const getWordContent = async (req, res) => {
    try {
        const { id, dokumenId } = req.params;
        const [docRows] = await pool.query(
            'SELECT id, nama_file, path, ukuran, uploaded_by FROM dokumen_upload WHERE id = ? AND is_deleted = 0',
            [dokumenId]
        );
        if (docRows.length === 0) {
            return res.status(404).json({ success: false, message: 'Dokumen tidak ditemukan atau sudah dihapus' });
        }

        const doc = docRows[0];
        const ext = path.extname(doc.nama_file || doc.path).toLowerCase();
        if (ext !== '.docx') {
            return res.status(400).json({ 
                success: false, 
                message: 'Hanya dokumen Microsoft Word (.docx) yang dapat diedit langsung di sistem.' 
            });
        }
        ...
        if (!filePath || !fs.existsSync(filePath)) {
            return res.status(404).json({ 
                success: false, 
                message: `File fisik dokumen tidak ditemukan di server (${doc.path})` 
            });
        }
        ...
        const [versions] = await pool.query(`
            SELECT nv.id, nv.versi, nv.nama_file, nv.path, nv.ukuran, nv.catatan_revisi, nv.created_at,
                   COALESCE(pp.nama_lengkap, u.username) as creator_name
            FROM notulensi_versi nv
            LEFT JOIN users u ON nv.created_by = u.id
            LEFT JOIN profil_pegawai pp ON u.profil_pegawai_id = pp.id
            WHERE nv.dokumen_id = ?
            ORDER BY nv.versi DESC
        `, [dokumenId]);
        ...
        console.error('[getWordContent] Error:', err);
        return res.status(500).json({ success: false, message: 'Gagal memuat konten dokumen: ' + err.message });
    }
};

const saveWordContent = async (req, res) => {
    const connection = await pool.getConnection();
    try {
        await connection.beginTransaction();
        const { id, dokumenId } = req.params;
        const { htmlContent, catatan_revisi } = req.body;
        const userId = req.user ? req.user.id : null;

        if (!htmlContent || typeof htmlContent !== 'string') {
            await connection.rollback();
            return res.status(400).json({ success: false, message: 'Konten dokumen tidak boleh kosong' });
        }
        ...
        // Fetch activity info
        const [kegiatanRows] = await connection.query(
            'SELECT id, nama_kegiatan FROM kegiatan_manajemen WHERE id = ?',
            [id]
        );
        ...
        // Check if notulensi_versi has any records for this doc.
        // If empty, save current version as Versi 1 (original snapshot)
        const [existingVersions] = await connection.query(
            'SELECT versi FROM notulensi_versi WHERE dokumen_id = ? ORDER BY versi DESC',
            [dokumenId]
        );
        ...
        if (existingVersions.length === 0) {
            await connection.query(`
                INSERT INTO notulensi_versi (kegiatan_id, dokumen_id, versi, nama_file, path, ukuran, catatan_revisi, created_by)
                VALUES (?, ?, 1, ?, ?, ?, 'Versi asli unggahan', ?)
            `, [id, dokumenId, doc.nama_file, doc.path, doc.ukuran || 0, doc.uploaded_by || userId]);
            currentMaxVersion = 1;
        } else {
            currentMaxVersion = existingVersions[0].versi;
        }
        ...
        const saveDir = path.join(__dirname, '../../../../uploads/kegiatan');
        if (!fs.existsSync(saveDir)) {
            fs.mkdirSync(saveDir, { recursive: true });
        }

        // Generate safe unique filename for new revision
        const safeName = (doc.nama_file || 'notulensi').replace(/\.[^/.]+$/, '').replace(/[^a-zA-Z0-9_\-]/g, '_');
        const newFileName = `${Date.now()}_v${nextVersion}_${safeName}.docx`;
        const newFilePath = path.join(saveDir, newFileName);
        const relativeDbPath = `/uploads/kegiatan/${newFileName}`;

        // Write new file safely
        fs.writeFileSync(newFilePath, docxBuffer);

        // Record new version in notulensi_versi
        await connection.query(`
            INSERT INTO notulensi_versi (kegiatan_id, dokumen_id, versi, nama_file, path, ukuran, catatan_revisi, created_by)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `, [id, dokumenId, nextVersion, doc.nama_file, relativeDbPath, docxBuffer.length, catatan_revisi || `Revisi konten v${nextVersion}`, userId]);

        // Update dokumen_upload with new path and size
        await connection.query(`
            UPDATE dokumen_upload 
            SET path = ?, ukuran = ?, updated_at = NOW() 
            WHERE id = ?
        `, [relativeDbPath, docxBuffer.length, dokumenId]);

        // Update kegiatan_manajemen_dokumen path
        await connection.query(`
            UPDATE kegiatan_manajemen_dokumen
            SET path = ?
            WHERE kegiatan_id = ? AND dokumen_id = ?
        `, [relativeDbPath, id, dokumenId]);

        // Record audit logs
        const logNote = catatan_revisi ? `Catatan: "${catatan_revisi}"` : 'Pembaruan teks notulensi langsung di sistem';
        await connection.query(
            'INSERT INTO dokumen_edit_history (dokumen_id, user_id, aksi, keterangan) VALUES (?, ?, "edit", ?)',
            [dokumenId, userId, `Revisi Word (v${nextVersion}). ${logNote}`]
        );

        await connection.query(
            'INSERT INTO kegiatan_edit_history (kegiatan_id, user_id, aksi, keterangan) VALUES (?, ?, "edit", ?)',
            [id, userId, `Dokumen notulensi "${doc.nama_file}" diperbarui ke Versi ${nextVersion}. ${logNote}`]
        );
        ...
        return res.json({
            success: true,
            message: `Dokumen notulensi berhasil disimpan ke Versi ${nextVersion}!`,
            data: {
                dokumen_id: parseInt(dokumenId),
                versi: nextVersion,
                path: relativeDbPath,
                ukuran: docxBuffer.length
            }
        });
    } catch (err) {
        await connection.rollback();
        console.error('[saveWordContent] Error:', err);
        return res.status(500).json({ success: false, message: 'Gagal menyimpan perubahan dokumen: ' + err.message });
    } finally {
        connection.release();
    }
};
...
const getWordHistory = async (req, res) => {
    try {
        ...
        console.error('[getWordHistory] Error:', err);
        return res.status(500).json({ success: false, message: 'Gagal mengambil riwayat versi: ' + err.message });
    }
};
