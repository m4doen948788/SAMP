...
const suratController = {
    ...
    generateSuratKeluar: async (req, res) => {
        ...
        try {
            ...
            // Link to activity if provided
            if (kegiatan_id) {
                let tipeDokumen = 'surat_undangan_keluar';
                if (jenis_surat_id) {
                    const [jsRows] = await connection.query('SELECT dokumen FROM master_dokumen WHERE id = ?', [jenis_surat_id]);
                    if (jsRows.length > 0 && jsRows[0].dokumen && jsRows[0].dokumen.toLowerCase().includes('perintah')) {
                        tipeDokumen = 'surat_perintah';
                    }
                }
                await connection.query(
                    'INSERT INTO kegiatan_manajemen_dokumen (kegiatan_id, nama_file, path, tipe_dokumen, dokumen_id) VALUES (?, ?, ?, ?, ?)',
                    [kegiatan_id, nomor_surat.replace(/\//g, '_') + '.docx', filePath, tipeDokumen, docResult.insertId]
                );
                // Sync thematic tagging
    ...
    update: async (req, res) => {
        ...
        try {
            ...
            // 1. Get current surat data (Full snapshot for activity log)
            ...
            if (finalDocId) {
                ...
                if (Number(normalizedKegId) !== Number(normalizedOldKegId)) {
                    ...
                    if (normalizedKegId) {
                        ...
                        if (docRows.length > 0) {
                            let tipeDok = (tipe_surat || 'masuk') === 'masuk' ? 'surat_undangan_masuk' : 'surat_undangan_keluar';
                            if (jenis_surat_id) {
                                const [jsRows] = await connection.query('SELECT dokumen FROM master_dokumen WHERE id = ?', [jenis_surat_id]);
                                if (jsRows.length > 0 && jsRows[0].dokumen && jsRows[0].dokumen.toLowerCase().includes('perintah')) {
                                    tipeDok = 'surat_perintah';
                                }
                            }
                            await connection.query(
                                'INSERT INTO kegiatan_manajemen_dokumen (kegiatan_id, nama_file, path, tipe_dokumen, dokumen_id) VALUES (?, ?, ?, ?, ?)',
