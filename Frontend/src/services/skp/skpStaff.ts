import { api } from '../api';
import { StaffTunggakan, monthNamesId, normalizeStr, matchPendukungDoc } from './skpCommon';

export const getDetailedStaffTunggakan = async (user: any): Promise<StaffTunggakan[]> => {
  if (!user || !user.bidang_id) return [];

  const cacheKey = `skp-staff-tunggakan-${user.id || user.profil_pegawai_id || 0}`;
  try {
  ...
  try {
    ...
    try {
      sessionStorage.setItem(cacheKey, JSON.stringify(tunggakanList));
