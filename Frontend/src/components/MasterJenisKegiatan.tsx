...
const MasterJenisKegiatan = () => {
    ...
    const [data, setData] = useState<JenisKegiatanItem[]>([]);
    ...
    const columns = [
        {
            header: getLabel('master_jenis_kegiatan', 'nama', 'Nama Jenis Kegiatan'),
            key: 'nama',
    ...
    return (
        <BaseDataTable<JenisKegiatanItem>
            title="Master Data Jenis Kegiatan"
            subtitle="Kelola kategori kegiatan dalam sistem."
            data={data}
            columns={columns}
            loading={loading}
            error={error}
            ...
            renderAddRow={() => isAdding && (
                <tr className="bg-blue-50">
                    ...
                    <td className="p-2 border-b border-slate-100">
                        <input autoFocus type="text" className="input-modern" placeholder="Masukkan jenis kegiatan..." value={newNama} onChange={e => setNewNama(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleAdd()} />
                    </td>
                    <td className="p-2 border-b border-slate-100">
