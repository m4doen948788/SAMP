...
const MasterJenisDokumen = () => {
    ...
    const [data, setData] = useState<JenisDokumenItem[]>([]);
    ...
    const columns = [
        {
            header: getLabel('master_jenis_dokumen', 'nama', 'Nama Jenis Dokumen'),
            key: 'nama',
    ...
    return (
        <BaseDataTable<JenisDokumenItem>
            title="Master Data Jenis Dokumen"
            subtitle="Kelola kategori dokumen yang dapat diunggah."
            data={data}
            columns={columns}
            loading={loading}
            error={error}
            ...
            renderAddRow={() => isAdding && (
                <tr className="bg-blue-50">
                    ...
                    <td className="p-2 border-b border-slate-100">
                        <input autoFocus type="text" className="input-modern" placeholder="Masukkan jenis dokumen..." value={newNama} onChange={e => setNewNama(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleAdd()} />
                    </td>
                    <td className="p-2 border-b border-slate-100">
