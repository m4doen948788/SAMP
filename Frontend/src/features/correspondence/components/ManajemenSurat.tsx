...
export default function ManajemenSurat({ onNavigate }: ManajemenSuratProps) {
    ...
    const [downloadingSuratId, setDownloadingSuratId] = useState<number | null>(null);
    ...
    const printOrDownloadHtmlSurat = async (surat: SuratItem) => {
        try {
            const loadingToast = toast.loading('Menyiapkan dokumen surat...');
            const res = await api.internalInstansi.get(user.instansi_id);
            let template = null;
            if (surat.jenis_surat_id) {
                const tRes = await api.suratTemplate.getById(surat.jenis_surat_id);
            ...
            if (printWindow) {
                ...
                let eventDataHtml = '';
                try {
                    const meta = typeof surat.metadata === 'string' ? JSON.parse(surat.metadata) : surat.metadata;
                    if (meta && meta.eventData) {
                        const ed = meta.eventData;
                        eventDataHtml = `
                            <div style="margin-top: 20px; font-family: Arial, sans-serif; font-size: 12pt;">
                ...
                printWindow.document.write(`
                    <html>
                        <head>
                            ...
                            <style>
                                body { 
                                    font-family: ${source.font_family || (useGlobal && globalSettings?.font_family) || 'Arial, sans-serif'}; 
                                    font-size: ${fSize}pt; 
            ...
            toast.error('Gagal menyiapkan cetakan dokumen');
        }
    };

    const handleDownloadSurat = async (surat: SuratItem) => {
        // Mode 1: Berkas fisik tersedia
        if (surat.file_path) {
            try {
                setDownloadingSuratId(surat.id);
                const token = sessionStorage.getItem('token');
                const rawName = surat.nama_file || `${(surat.nomor_surat || 'surat').replace(/[/\\?%*:|"<>]/g, '_')}.pdf`;
                const fileName = rawName.includes('.') ? rawName : `${rawName}.pdf`;
                
                let blob: Blob | null = null;
                try {
                    const res = await fetch(`${API_URL}/dokumen/download-by-path?filePath=${encodeURIComponent(surat.file_path)}`, {
                    ...
                    console.warn('download-by-path failed, fallback to direct fetch:', e);
                }

                if (!blob) {
                    let directUrl = surat.file_path;
                    if (!directUrl.startsWith('http') && !directUrl.startsWith('/')) {
                        directUrl = '/' + directUrl;
                    }
                    const res = await fetch(directUrl);
                    if (!res.ok) throw new Error('File fisik tidak dapat diunduh dari server.');
                    blob = await res.blob();
                }
                ...
                toast.error(error.message || 'Gagal mengunduh berkas surat');
            } finally {
        ...
        // Mode 2: Surat digital / draft sistem yang memiliki isi_surat
        if (surat.isi_surat) {
            if (surat.tipe_surat === 'keluar' && surat.approval_status !== 'APPROVED') {
                toast.error('Surat keluar belum dapat diunduh karena masih menunggu persetujuan.');
                return;
            }
            await printOrDownloadHtmlSurat(surat);
            return;
        }

        // Mode 3: Belum ada berkas fisik maupun draft sistem
        toast.error('Dokumen surat belum tersedia untuk diunduh. Silakan unggah berkas fisik terlebih dahulu.');
    };
    ...
    return (
        <div className="space-y-2.5 p-4 pt-2">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-2">
                    <div className="w-7 h-7 bg-ppm-slate-light rounded-lg flex items-center justify-center text-white shadow-lg shadow-ppm-slate-light/20 shrink-0">
                        <Mail size={14} />
                    </div>
                    <div>
                    ...
                    <div className="flex bg-slate-100/80 p-0.5 rounded-xl border border-slate-200/50 shadow-inner shrink-0">
                        <button 
                            ...
                            className={`px-4 h-7 rounded-lg font-black transition-all text-[9px] uppercase tracking-widest ${
                                activeTab === 'masuk' ? 'bg-ppm-slate-light text-white shadow-sm' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-200/30'
                            }`}
                        >
                            Surat Masuk
                        </button>
                        <button 
                            ...
                            className={`px-4 h-7 rounded-lg font-black transition-all text-[9px] uppercase tracking-widest ${
                                activeTab === 'keluar' ? 'bg-ppm-slate-light text-white shadow-sm' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-200/30'
                            }`}
                        >
                            Surat Keluar
                        </button>
                        <button 
                            ...
                            className={`px-4 h-7 rounded-lg font-black transition-all text-[9px] uppercase tracking-widest ${
                                activeTab === 'internal' ? 'bg-ppm-slate-light text-white shadow-sm' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-200/30'
                            }`}
                        >
                            Surat Internal
                        </button>
                        ...
                        <div className="flex bg-slate-100/80 p-0.5 rounded-xl border border-slate-200/50 shadow-inner h-8 shrink-0">
                            <button 
                                ...
                                className={`px-3.5 h-7 rounded-lg font-black transition-all text-[9px] uppercase tracking-widest flex items-center gap-1.5 ${
                                    viewTrash === 'active' ? 'bg-ppm-slate-light text-white shadow-sm' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-200/30'
                                }`}
                            >
                                <Database size={11} />
                                Aktif
                                ...
                                {displayedSurat.map((surat) => (
                                    <tr key={surat.id} className={`group/row transition-all ${surat.is_deleted ? 'bg-slate-50/40 opacity-60 grayscale-[0.5]' : 'hover:bg-slate-50/80'}`}>
                                        ...
                                        <td className="px-4 py-3">
                                            ...
                                            >
                                                <div className="p-2 bg-ppm-slate-light/10 rounded-lg text-ppm-slate-light group-hover/file:bg-ppm-slate-light group-hover/file:text-white transition-all shadow-sm">
                                                    <FileText size={16} />
                                                </div>
                                                <div className="min-w-0">
                                                ...
                                                {viewTrash === 'trash' ? (
                                                    ...
                                                    <>
                                                        ...
                                                        {(!surat.file_path && surat.tipe_surat === 'keluar' && surat.approval_status !== 'APPROVED') ? (
                                                            <button 
                                                                className="p-2 bg-ppm-slate-light/5 text-ppm-slate-light/30 rounded-xl cursor-not-allowed" 
                                                            ...
                                                            <button 
                                                                ...
                                                                disabled={downloadingSuratId === surat.id}
                                                                className="p-2 bg-ppm-slate-light/10 text-ppm-slate-light hover:bg-ppm-slate hover:text-white rounded-xl transition-all disabled:opacity-50" 
                                                                title={surat.file_path ? "Unduh Berkas Fisik" : (surat.isi_surat ? "Cetak / Unduh PDF Surat" : "Unduh Surat")}
                                                            >
                                                                {downloadingSuratId === surat.id ? (
                                                                    <Loader2 size={18} className="animate-spin text-ppm-blue" />
                                                                ) : (
                                                                    <Download size={18} />
                                                                )}
                                                            </button>
                                                        ...
                                                        <div className="relative">
                                                            <button 
                                                                ...
                                                                className={`p-2 rounded-xl transition-all ${activeMenuId === surat.id ? 'bg-ppm-slate-light text-white shadow-sm' : 'bg-ppm-slate-light/10 text-ppm-slate-light hover:bg-ppm-slate-light/20'}`}
                                                            >
                                    ...
                                    {[...Array(totalPages)].map((_, i) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`w-8 h-8 rounded-lg text-[11px] font-extrabold transition-all ${currentPage === page ? 'bg-ppm-slate-light text-white shadow-lg' : 'text-ppm-slate-light hover:bg-ppm-slate-light/20'}`}
                                            >
                                                {page}
                                            </button>
                                        );
                                ...
                                {canEdit && (
                                    ...
                                    >
                                        ...
                                        Ubah Data
                                                                 ...
                                                                 {(surat.file_path || surat.isi_surat) && (
                                    <button 
                                        onClick={async () => {
                                            setActiveMenuId(null);
                                            await handleDownloadSurat(surat);
                                        }}
                                        className="w-full px-4 py-2.5 text-left text-xs font-black text-slate-600 hover:bg-slate-50 flex items-center gap-2 transition-colors"
                                    >
                                        <Printer size={14} className="text-amber-500" />
                                        {surat.file_path ? 'Unduh Berkas' : 'Cetak / Unduh PDF'}
                                    </button>
                                )}
                                {canDelete && (
                                    <button 
